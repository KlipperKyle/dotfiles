...
#
# Some modifications:
#  Kyle Terrien <kyleterrien@gmail.com>
#
...
DHCPRANGE=192.168.53.2,192.168.53.127
...
# I have merged these into /etc/iptables by hand.
# I don't like other daemons touching my iptables configuration.
add_filter_rules() {
...
check_dnsmasq() {
    if [ -f /var/run/qemu-dnsmasq-${1}.pid ] ; then
	if ps $(cat /var/run/qemu-dnsmasq-${1}.pid) > /dev/null ; then
	    return 1
	else
	    return 0
	fi
...
setup_bridge_nat() {
	...
	create_bridge "$1"
	# Handled by Slackware's rc.ip_forward
	#enable_ip_forward
	# Don't touch my iptables configuration!
	#add_filter_rules "$1"
    fi
    if check_dnsmasq "$1" ; then
	start_dnsmasq "$1"
    fi
}
